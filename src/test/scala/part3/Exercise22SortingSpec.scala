...
class Exercise22SortingSpec extends AnyFlatSpec with Matchers {
  ...
  "filmsSortedByImdb" should "do the right thing" in {
    filmsSortedByImdb(nolan.films) should equal(
      List(memento, inception, darkKnight)
    )
    filmsSortedByImdb(eastwood.films) should equal(
      List(
        invictus,
        highPlainsDrifter,
        outlawJoseyWales,
  ...
  "filmsByDirectorSortedByImdb" should "do the right thing" in {
    filmsByDirectorSortedByImdb(nolan) should equal(
      List(memento, inception, darkKnight)
    )
    filmsByDirectorSortedByImdb(eastwood) should equal(
      List(
        invictus,
        highPlainsDrifter,
        outlawJoseyWales,
  ...
  "filmsByAllDirectorsSortedByImdb" should "do the right thing" in {
    filmsByAllDirectorsSortedByImdb(List(nolan, mcTiernan)) should equal(
      List(
        thomasCrownAffair,
        huntForRedOctober,
        predator,
  ...
  "filmsByAllDirectorsSortedByDirectorNameThenImdb" should "do the right thing" in {
    filmsByAllDirectorsSortedByDirectorNameThenImdb(List(nolan, mcTiernan)) should equal(
      List(
        memento,
        inception,
        darkKnight,
