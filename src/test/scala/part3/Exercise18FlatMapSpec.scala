...
class Exercise18FlatMapSpec extends AnyFlatSpec with Matchers {
  ...
  "filmsByDirector" should "do the right thing" in {
    filmsByDirector(nolan) should contain theSameElementsAs List(
      memento,
      darkKnight,
      inception
    )
  ...
  "namesOfFilmsByDirector" should "do the right thing" in {
    namesOfFilmsByDirector(nolan) should contain theSameElementsAs List(
      memento.name,
      darkKnight.name,
      inception.name
    )
  ...
  "namesOfFilmsByAllDirectors" should "do the right thing" in {
    namesOfFilmsByAllDirectors(List(nolan, mcTiernan)) should contain theSameElementsAs (nolan.films ++ mcTiernan.films)
      .map(f => f.name)
    namesOfFilmsByAllDirectors(List(eastwood, someGuy)) should contain theSameElementsAs eastwood.films
      .map(f => f.name)
  }
