...
class Exercise17FoldLeftAndRightSpec extends AnyFlatSpec with Matchers {
  ...
  "totalImdbRating" should "do the right thing" in {
    totalImdbRating(List(memento, outlawJoseyWales, thomasCrownAffair)) should equal(
      memento.imdbRating + outlawJoseyWales.imdbRating + thomasCrownAffair.imdbRating
    )
    totalImdbRating(Nil) should equal(0.0)
  }

  "averageImdbRating" should "do the right thing" in {
    averageImdbRating(List(memento, outlawJoseyWales, thomasCrownAffair)) should equal(
      (memento.imdbRating + outlawJoseyWales.imdbRating + thomasCrownAffair.imdbRating) / 3.0
    )
  ...
  "filterUsingFold" should "do the right thing" in {
    filterUsingFold(List(1, 2, 3, 4, 5), (n: Int) => n % 2 == 0) should equal(
      List(2, 4)
    )
