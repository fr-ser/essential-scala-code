...
class Exercise8aVecSpec extends AnyFlatSpec with Matchers {
  ...
  "length" should "calculate the correct value" in {
    ...
    Vec.length should equal(5)
    vec2.length should equal(13)
  }

  "+" should "calculate the correct value" in {

    (Vec + vec2) should equal(new Vec(8, 16))
    (Vec + vec2).length should equal(math.sqrt(8 * 8 + 16 * 16))
  }

  "*" should "calculate the correct value" in {

    // (Vec * 10) should equal(new Vec(30, 40))
    // (Vec * 10).length should equal(50)
  }
  ...
  "longest(Vec, Vec)" should "calculate the correct value" in {
    ...
    // Vec.longest(Vec, vec2) should equal(vec2)
